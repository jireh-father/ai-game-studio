...
| **SMOOSH!** — infinite-stage jelly tap-slaughter grown into a full tap-RPG (nest defense, 50 animal pets, gacha, gear, gems, PvP, dormant IAP), Play release under the pending ORG account | `games-for-release/smoosh-mobile/RELEASE_STATUS.md` | Capacitor Android project (package `com.represen.smoosh`), built 2026-07-02, heavily evolved 07-02~03 through **v2.5**: nest defend-target (break = stage retry), 50 parametric animal pets ALL deployed per stage, per-species monster attacks (melee/spit/spray/charge/slam/zap), gacha w/ pity+shards, 3-slot gear, gems + dormant IAP facade, live same-field PvP vs bots, in-game pause-overlay shop, 7-type item drops, synthesized monster voices. **STATUS: BUILD READY** — signed AAB w/ REAL AdMob IDs (app `~5373653709` under `pub-7114194646987493`, `USE_TEST_ADS:true`), 62/62 tests incl. 200-stage balance-invariant sim (HP 1.25^n / cost 1.352). Only gate: org Play account. Full version log in the status doc. Spec/plan: `docs/superpowers/{specs,plans}/2026-07-02-smoosh-*.md`. Own LOCAL git repo inside the folder. |
