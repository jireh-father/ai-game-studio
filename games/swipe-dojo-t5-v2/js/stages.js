...
function buildSequence(length, windowMs, fakeoutChance, stageNum) {
  const seq = [];
  let prevDir = null;
  for (let i = 0; i < length; i++) {
    ...
    // Multi-arrow burst: at stage 8+, 15% chance per attack
    let burstCount = 0;
    if (stageNum >= BURST.MIN_STAGE && Math.random() < BURST.CHANCE) {
      burstCount = Phaser.Math.Between(BURST.MIN_ARROWS, BURST.MAX_ARROWS);
    }
    seq.push({
      direction: dir,
      windowMs: windowMs,
...
function generateStage(stageNum) {
  ...
  const sequence = buildSequence(seqLen, attackWindow, fakeout, stageNum);
