...
class AudioSynth {
  ...
  playFinishingBlow() {
    if (!this.initialized || !this.sfxEnabled) return;
    this._playTone(880, 0.15, 'square', 0.3);
    this._playTone(1100, 0.2, 'sine', 0.25);
    this._playNoise(0.1, 0.2);
    setTimeout(() => this._playTone(1320, 0.25, 'sine', 0.3), 150);
  }

  playComeback() {
    if (!this.initialized || !this.sfxEnabled) return;
    const notes = [440, 554, 659, 880];
    notes.forEach((f, i) => setTimeout(() => this._playTone(f, 0.12, 'sine', 0.2), i * 80));
  }

  playOverkill() {
    if (!this.initialized || !this.sfxEnabled) return;
    this._playTone(200, 0.15, 'sawtooth', 0.3);
    this._playTone(400, 0.15, 'square', 0.25);
    this._playNoise(0.15, 0.25);
    setTimeout(() => this._playTone(800, 0.2, 'sine', 0.3), 100);
  }
