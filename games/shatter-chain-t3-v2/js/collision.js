...
const CollisionMixin = {
  ...
  shatterPanel(glassBody, chainDepth, ix, iy) {
    ...
    // Score (with achievement multiplier)
    const basePts = CFG.CHAIN_SCORE[Math.min(chainDepth, 3)];
    const pts = Math.round(basePts * MetaProgress.getScoreMultiplier());
    ...
    if (chainDepth >= 1) { this.fx.chainCombo(chainDepth); MetaProgress.addCascade(); }
    if (chainDepth >= 4) this.fx.slowMotion(400, 0.7);
    this.fx.screenCrack(ix, iy, chainDepth);
    MetaProgress.addShards(1);
  ...
  spawnShards(glassBody, chainDepth, ix, iy) {
    ...
    if (gs._dailyMod && gs._dailyMod.shardCountMult) shardCount = Math.round(shardCount * gs._dailyMod.shardCountMult);
    ...
    for (let i = 0; i < shardCount; i++) {
      ...
      if (gs._dailyMod && gs._dailyMod.shardSpeedMult) speed *= gs._dailyMod.shardSpeedMult;
      const size = 5 + Math.random() * 8;

      const shardOpts = { ...SHARD_BODY, plugin: { chainDepth: chainDepth } };
      if (gs._dailyMod && gs._dailyMod.shardGravMult) shardOpts.frictionAir = SHARD_BODY.frictionAir / gs._dailyMod.shardGravMult;
