...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    const dailyMod = gs.is_daily_challenge ? gs._dailyMod : null;
    const wave = generateWave(gs.waveNumber, gs.sessionSeed, dailyMod);
  ...
  createHUD() {
    ...
    const streak = MetaProgress.data.streak, mult = MetaProgress.getScoreMultiplier();
    if (streak > 0) this.add.text(10, 32, '\uD83D\uDD25'+streak, { fontSize:'12px', fontFamily:'Arial', color:'#FF6600' }).setDepth(300);
    if (mult > 1) this.add.text(CFG.WIDTH-10, 28, `x${mult.toFixed(1)}`, { fontSize:'11px', fontFamily:'Arial', fontStyle:'bold', color:'#FFD700' }).setOrigin(1,0).setDepth(300);
    if (gs.is_daily_challenge) this.add.text(CFG.WIDTH/2, 32, 'DAILY: '+gs._dailyMod.label, { fontSize:'11px', fontFamily:'Arial', color:'#FF9900' }).setOrigin(0.5,0).setDepth(300);
