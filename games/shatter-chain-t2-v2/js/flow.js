...
const FlowMixin = {
  ...
  onWaveClear() {
    ...
    // Persist tracking across wave restarts
    window.GameState._totalChains = this.totalChainsTriggered;
    window.GameState._totalPanels = this.totalPanelsDestroyed;
    window.GameState._bestLaunchPanels = this.bestSingleLaunchPanels;
  ...
  gameOver() {
    const gs = window.GameState;
    const isNewHigh = gs.score > gs.highScore;
    if (isNewHigh) {
      gs.highScore = gs.score;
      localStorage.setItem('shatter-chain_high_score', gs.highScore);
    }
    const hw = parseInt(localStorage.getItem('shatter-chain_highest_wave') || '0');
    const newWaveRecord = gs.waveNumber > hw;
    if (newWaveRecord) localStorage.setItem('shatter-chain_highest_wave', gs.waveNumber);

    const bd = parseInt(localStorage.getItem('shatter-chain_best_chain_depth') || '0');
    const newChainRecord = this.maxChainDepth > bd;
    if (newChainRecord) localStorage.setItem('shatter-chain_best_chain_depth', this.maxChainDepth);

    const bp = parseInt(localStorage.getItem('shatter-chain_best_launch_panels') || '0');
    const newLaunchRecord = this.bestSingleLaunchPanels > bp;
    if (newLaunchRecord) localStorage.setItem('shatter-chain_best_launch_panels', this.bestSingleLaunchPanels);

    AdManager.onGameOver();
    this.scene.start('GameOverScene', {
      score: gs.score,
      wave: gs.waveNumber,
      chainDepth: this.maxChainDepth,
      isNewHigh: isNewHigh,
      totalPanelsDestroyed: this.totalPanelsDestroyed,
      totalChainsTriggered: this.totalChainsTriggered,
      bestSingleLaunchPanels: this.bestSingleLaunchPanels,
      newWaveRecord: newWaveRecord,
      newChainRecord: newChainRecord,
      newLaunchRecord: newLaunchRecord,
      pbWave: Math.max(gs.waveNumber, hw),
      pbChain: Math.max(this.maxChainDepth, bd),
      pbLaunchPanels: Math.max(this.bestSingleLaunchPanels, bp),
    });

    // Clean up tracking
    gs._totalChains = 0;
    gs._totalPanels = 0;
    gs._bestLaunchPanels = 0;
  },
};
