...
function getDifficultyParams(wave) {
  ...
  let bombRatio = wave >= 6 ? 0.08 : 0;
  let iceRatio = wave >= 10 ? 0.12 : 0;
  ...
  return { panelCount, balls, timer, reinforcedRatio, armoredRatio, bombRatio, iceRatio, barrierCount, gapMin, moving, isRest, isBoss };
}
...
function generateWave(waveNumber, sessionSeed) {
  ...
  panels.forEach(p => {
    const r = rng();
    if (r < diff.bombRatio) { p.type = 'bomb'; p.hp = 1; p.w = 42; p.h = 32; }
    else if (r < diff.bombRatio + diff.iceRatio) { p.type = 'ice'; p.hp = 1; p.w = 40; p.h = 30; }
    else if (r < diff.bombRatio + diff.iceRatio + diff.armoredRatio) { p.type = 'armored'; p.hp = 3; p.w = 48; p.h = 38; }
    else if (r < diff.bombRatio + diff.iceRatio + diff.armoredRatio + diff.reinforcedRatio) { p.type = 'reinforced'; p.hp = 2; p.w = 44; p.h = 34; }
