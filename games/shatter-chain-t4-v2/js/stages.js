...
function generateWave(waveNumber, sessionSeed) {
  ...
  if (waveNumber >= 8 && waveNumber <= 15) {
    gravZones.push({ y: CFG.ARENA_TOP + CFG.ARENA_HEIGHT / 2 });
  } else if (waveNumber >= 16) {
    gravZones.push({ y: CFG.ARENA_TOP + CFG.ARENA_HEIGHT / 3 });
    gravZones.push({ y: CFG.ARENA_TOP + CFG.ARENA_HEIGHT * 2 / 3 });
  }

  return { panels, barriers, gravZones, diff };
}
