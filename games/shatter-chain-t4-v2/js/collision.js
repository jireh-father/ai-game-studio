...
const CollisionMixin = {
  onCollision(event) {
    ...
    for (const pair of pairs) {
      ...
      if (glass && (ball || shard)) {
        ...
        // Ball personality: impact squash
        if (ball && this.ballSprite) {
          this.fx.ballImpactSquash(this.ballSprite);
        }
      }
    }
  },
  ...
  hitGlass(glassBody, chainDepth, impactPos) {
    ...
    this.shatterPanel(glassBody, chainDepth, ix, iy, glassBody._fusionShardBonus || 0);
  },

  shatterPanel(glassBody, chainDepth, ix, iy, fusionBonus) {
    ...
    this.spawnShards(glassBody, chainDepth, ix, iy, fusionBonus || 0);
  ...
  spawnShards(glassBody, chainDepth, ix, iy, fusionBonus) {
    const worldBodies = this.matter.world.localWorld.bodies.length;
    let shardCount = CFG.SHARD_BASE_COUNT + Math.min(chainDepth, 4) * 2 + (fusionBonus || 0);
