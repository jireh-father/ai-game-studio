...
const FlowMixin = {
  onBallLost() {
    ...
    this.removeBall();
    playBallLost();
    // Reset aim preview state so it can reappear
  ...
  onWaveClear() {
    ...
    playWaveClear();
    // Ball wave clear spin celebration before removing
    if (this.ballSprite && this.ballInFlight) {
      this.fx.ballClearSpin(this.ballSprite);
      this.time.delayedCall(CFG.BALL_CLEAR_DURATION, () => this.removeBall());
    } else {
      this.removeBall();
    }
  ...
  gameOver() {
    ...
    this.scene.start('GameOverScene', {
      ...
      isNewHigh: gs.score >= gs.highScore
    });
  },
};
