...
class EffectsManager {
  ...
  floatScore(x, y, points, chainDepth, isGoldRush) {
    const color = isGoldRush ? CFG.COLOR.GOLDEN_HEX : CFG.CHAIN_COLORS[Math.min(chainDepth, 4)];
    const size = 20 + Math.min(chainDepth, 4) * 4 + (isGoldRush ? 4 : 0);
    let label = chainDepth > 0 ? `+${points} x${Math.pow(2, Math.min(chainDepth, 3))}` : `+${points}`;
    if (isGoldRush) label += ' GOLD';
  ...
  // Gold Rush flash + text
  goldRushFlash() {
    const flash = this.scene.add.rectangle(CFG.WIDTH / 2, CFG.HEIGHT / 2, CFG.WIDTH, CFG.HEIGHT, CFG.COLOR.GOLDEN, 0.35).setDepth(200);
    this.scene.tweens.add({ targets: flash, alpha: 0, duration: 400, onComplete: () => flash.destroy() });

    const txt = this.scene.add.text(CFG.WIDTH / 2, CFG.ARENA_TOP + CFG.ARENA_HEIGHT / 2 - 30, '3X GOLD RUSH!', {
      fontSize: '42px', fontFamily: 'Arial', fontStyle: 'bold',
      color: CFG.COLOR.GOLDEN_HEX, stroke: '#000', strokeThickness: 4,
    }).setOrigin(0.5).setDepth(210).setScale(0.3);

    this.scene.tweens.add({
      targets: txt, scaleX: 1.2, scaleY: 1.2, duration: 150,
      ease: 'Back.easeOut', hold: 600,
      onComplete: () => {
        this.scene.tweens.add({ targets: txt, alpha: 0, y: txt.y - 30, duration: 300, onComplete: () => txt.destroy() });
      }
    });
    this.sparks(CFG.WIDTH / 2, CFG.ARENA_TOP + CFG.ARENA_HEIGHT / 2, 4, 30);
  }

  // Screen Crack overlay at chain depth 4+
  screenCrack(ix, iy, chainDepth) {
    const g = this.scene.add.graphics().setDepth(180);
    if (chainDepth >= 6) {
      // Full-screen shatter: 6 triangular overlays separate outward then snap back
      const cx = CFG.WIDTH / 2, cy = CFG.HEIGHT / 2;
      const triangles = [];
      for (let i = 0; i < CFG.SHATTER_OVERLAY_TRIANGLES; i++) {
        const angle = (i / CFG.SHATTER_OVERLAY_TRIANGLES) * Math.PI * 2;
        const nextAngle = ((i + 1) / CFG.SHATTER_OVERLAY_TRIANGLES) * Math.PI * 2;
        const tri = this.scene.add.triangle(
          cx, cy,
          0, 0,
          Math.cos(angle) * CFG.WIDTH, Math.sin(angle) * CFG.HEIGHT,
          Math.cos(nextAngle) * CFG.WIDTH, Math.sin(nextAngle) * CFG.HEIGHT,
          CFG.COLOR.WHITE, 0.08
        ).setDepth(185);
        const ox = Math.cos(angle + (nextAngle - angle) / 2) * CFG.SHATTER_OVERLAY_OFFSET;
        const oy = Math.sin(angle + (nextAngle - angle) / 2) * CFG.SHATTER_OVERLAY_OFFSET;
        this.scene.tweens.add({
          targets: tri, x: cx + ox, y: cy + oy, duration: 80,
          yoyo: true, ease: 'Quad.easeOut',
    ...
    if (chainDepth >= 5) {
      // Chromatic-like camera offset
      const cam = this.scene.cameras.main;
      cam.setScroll(CFG.CRACK_SHIFT_PX, 0);
      setTimeout(() => { cam.setScroll(0, 0); }, 60);
    }

    // Hairline crack lines radiating from impact point
    const lineCount = chainDepth >= 5 ? CFG.CRACK_DEPTH5_LINES : CFG.CRACK_DEPTH4_LINES;
    g.lineStyle(1, CFG.COLOR.WHITE, 0.3);
    for (let i = 0; i < lineCount; i++) {
      const angle = (i / lineCount) * Math.PI * 2 + Math.random() * 0.5;
      const len = 40 + Math.random() * 80;
      g.lineBetween(ix, iy, ix + Math.cos(angle) * len, iy + Math.sin(angle) * len);
    }
    this.scene.tweens.add({
      targets: g, alpha: 0, duration: CFG.CRACK_FADE_MS,
      onComplete: () => g.destroy()
    });
  }

  // Ball wave clear celebration spin
  ballClearSpin(ballSprite) {
    if (!ballSprite) return;
    this.scene.tweens.add({
      targets: ballSprite,
      angle: CFG.BALL_CLEAR_SPIN,
      scaleX: { value: 1.3, duration: CFG.BALL_CLEAR_DURATION / 2, yoyo: true },
      scaleY: { value: 1.3, duration: CFG.BALL_CLEAR_DURATION / 2, yoyo: true },
      duration: CFG.BALL_CLEAR_DURATION,
      ease: 'Power2',
    });
  }
}
