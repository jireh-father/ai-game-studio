...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.goldenDecided = false; // has any panel been hit yet this wave?
    this.absorberBody = null;
    wave.panels.forEach((p, i) => {
      const isGolden = (i === wave.goldenIndex);
      this.createGlassPanel(p, isGolden);
    });
    ...
    this._idleHopTween = null;
    this._aimBallPreview = null;
  ...
  createGlassPanel(p, isGolden) {
    ...
    const color = isGolden ? CFG.COLOR.GOLDEN :
      p.type === 'armored' ? CFG.COLOR.ARMORED : p.type === 'reinforced' ? CFG.COLOR.REINFORCED : CFG.COLOR.GLASS;
    const sprite = this.add.rectangle(p.x, p.y, w, h, color, isGolden ? 1.0 : (p.type === 'normal' ? 0.85 : 0.9)).setDepth(20);
    ...
    body._w = w;
    body._h = h;
    body._isGolden = !!isGolden;
    body._isAbsorber = false;

    if (isGolden) {
      this.goldenBody = body;
      // Pulsing golden glow
      const glow = this.add.rectangle(p.x, p.y, w + 8, h + 8, CFG.COLOR.GOLDEN_GLOW, 0.3).setDepth(19);
      this.tweens.add({ targets: glow, alpha: 0.6, scaleX: 1.15, scaleY: 1.15, duration: 500, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
      sprite._goldenGlow = glow;
      // On wave 8+ golden panel moves with sine wave
      const gs = window.GameState;
      if (gs.waveNumber >= CFG.GOLDEN_MOVE_WAVE) {
        body.plugin.moving = true;
        body.plugin.moveSpeed = CFG.GOLDEN_MOVE_SPEED;
        body.plugin.moveAmp = CFG.GOLDEN_MOVE_AMP;
        body.plugin.baseX = p.x;
      }
    }
  ...
  onPointerDown(pointer) {
    ...
    this.lastInputTime = this.time.now;
    // Cancel idle hop
    if (this._idleHopTween) { this._idleHopTween.stop(); this._idleHopTween = null; }
    // Ensure preview ball is at rest position for squash
    if (this._aimBallPreview) this._aimBallPreview.setPosition(CFG.WIDTH / 2, CFG.LAUNCH_ZONE_TOP - 20);
  }
  ...
  onPointerMove(pointer) {
    ...
    this.lastInputTime = this.time.now;
    // Ball personality: squash during drag-aim
    if (this._aimBallPreview) {
      const dx = pointer.x - this.aimStartX, dy = pointer.y - this.aimStartY;
      const dist = Math.sqrt(dx * dx + dy * dy);
      const t = Phaser.Math.Clamp(dist / 100, 0, 1); // 0..1 proportion
      const sx = 1 - t * (1 - CFG.BALL_AIM_SQUASH_X);
      const sy = 1 + t * (CFG.BALL_AIM_STRETCH_Y - 1);
      this._aimBallPreview.setScale(sx, sy);
    }
  }
  ...
  onPointerUp(pointer) {
    ...
    const dist = Math.sqrt(dx * dx + dy * dy);
    if (dist < CFG.BALL_DRAG_MIN) {
      // Reset aim ball preview scale
      if (this._aimBallPreview) this._aimBallPreview.setScale(1, 1);
      return;
    }
  ...
  launchBall(dx, dy, dist) {
    ...
    // Remove aim preview ball
    if (this._aimBallPreview) { this._aimBallPreview.destroy(); this._aimBallPreview = null; }
    if (this._idleHopTween) { this._idleHopTween.stop(); this._idleHopTween = null; }
  ...
  update(time, delta) {
    ...
    for (const body of this.glassBodies) {
      ...
      if (body._sprite) {
        ...
        if (body._sprite._goldenGlow) body._sprite._goldenGlow.setPosition(body.position.x, body.position.y);
      }
    }
    ...
    // Ball idle hop: when not in flight and idle > 3s, show a waiting ball that hops
    if (!this.ballInFlight && window.GameState.ballsLeft > 0 && this.waveTimerActive) {
      if (!this._aimBallPreview) {
        this._aimBallPreview = this.add.circle(CFG.WIDTH / 2, CFG.LAUNCH_ZONE_TOP - 20, CFG.BALL_RADIUS, CFG.COLOR.BALL).setDepth(30);
        this._aimBallPreview.setStrokeStyle(1.5, 0x888890);
        this._aimBallPreviewTime = time;
      }
      const idleTime = time - Math.max(this.lastInputTime, this._aimBallPreviewTime || 0);
      if (idleTime > CFG.BALL_IDLE_THRESHOLD && !this._idleHopTween) {
        this._idleHopTween = this.tweens.add({
          targets: this._aimBallPreview, y: CFG.LAUNCH_ZONE_TOP - 20 - CFG.BALL_IDLE_HOP,
          duration: CFG.BALL_IDLE_HOP_MS, ease: 'Sine.InOut', yoyo: true, repeat: -1,
        });
      }
    }
    // Cancel idle hop on input
    if (this.isAiming && this._idleHopTween) {
      this._idleHopTween.stop();
      this._idleHopTween = null;
    }

    // Update absorber glow + skull position
    if (this.absorberBody && this.absorberBody._sprite) {
      const abs = this.absorberBody;
      if (abs._sprite._absorberGlow) abs._sprite._absorberGlow.setPosition(abs.position.x, abs.position.y);
      if (abs._sprite._skull) abs._sprite._skull.setPosition(abs.position.x, abs.position.y);
    }
