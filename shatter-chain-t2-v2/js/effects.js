...
class EffectsManager {
  ...
  // Bomb explosion AoE effect
  bombExplosion(x, y) {
    // Expanding orange ring
    const ring = this.scene.add.circle(x, y, 10, CFG.COLOR.BOMB, 0).setDepth(95);
    ring.setStrokeStyle(4, CFG.COLOR.BOMB, 0.9);
    this.scene.tweens.add({
      targets: ring, scaleX: 8, scaleY: 8, alpha: 0, duration: 400,
      onComplete: () => ring.destroy()
    });
    // Flash
    const flash = this.scene.add.circle(x, y, 40, CFG.COLOR.BOMB, 0.6).setDepth(94);
    this.scene.tweens.add({
      targets: flash, scaleX: 3, scaleY: 3, alpha: 0, duration: 250,
      onComplete: () => flash.destroy()
    });
    // Orange sparks
    this.sparks(x, y, 0, CFG.BOMB_SPARK_COUNT);
    // Extra orange sparks
    for (let i = 0; i < CFG.BOMB_SPARK_COUNT; i++) {
      const angle = Math.random() * Math.PI * 2;
      const speed = 120 + Math.random() * 250;
      const r = 2 + Math.random() * 4;
      const spark = this.scene.add.circle(x, y, r, CFG.COLOR.BOMB, 0.95).setDepth(96);
      this.scene.tweens.add({
        targets: spark,
        x: x + Math.cos(angle) * speed * 0.8,
        y: y + Math.sin(angle) * speed * 0.8,
  ...
  // Ice freeze visual at impact point
  iceFreeze(x, y) {
    // Blue frost overlay
    const frost = this.scene.add.circle(x, y, 30, CFG.COLOR.ICE, 0.5).setDepth(95);
    this.scene.tweens.add({
      targets: frost, scaleX: 2.5, scaleY: 2.5, alpha: 0, duration: CFG.ICE_FREEZE_DURATION,
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const r = 2 + Math.random() * 3;
      const crystal = this.scene.add.circle(x, y, r, CFG.COLOR.ICE, 0.9).setDepth(96);
      this.scene.tweens.add({
        targets: crystal,
  ...
  // Demolition event title card
  demolitionEvent() {
    const txt = this.scene.add.text(CFG.WIDTH / 2, CFG.HEIGHT / 2, 'DEMOLITION!', {
      fontSize: '56px', fontFamily: 'Arial', fontStyle: 'bold',
      color: CFG.COLOR.GOLD_HEX, stroke: '#000', strokeThickness: 4,
    ...
    this.scene.tweens.add({
      targets: txt, scaleX: 1.0, scaleY: 1.0, duration: 300, delay: 150,
      yoyo: true, hold: 800,
      onComplete: () => {
        this.scene.tweens.add({ targets: txt, alpha: 0, y: txt.y - 50, duration: 400, onComplete: () => txt.destroy() });
      }
    });

    // Gold burst particles
    this.sparks(CFG.WIDTH / 2, CFG.HEIGHT / 2, 0, 40);
    for (let i = 0; i < 30; i++) {
      const angle = Math.random() * Math.PI * 2;
      const speed = 100 + Math.random() * 200;
      const r = 2 + Math.random() * 4;
      const spark = this.scene.add.circle(CFG.WIDTH / 2, CFG.HEIGHT / 2, r, CFG.COLOR.GOLD, 0.9).setDepth(96);
      this.scene.tweens.add({
        targets: spark,
        x: CFG.WIDTH / 2 + Math.cos(angle) * speed,
        y: CFG.HEIGHT / 2 + Math.sin(angle) * speed,
        alpha: 0, scaleX: 0, scaleY: 0,
