...
function isRageStage(stageNum) {
  return stageNum > 1 && stageNum % RAGE.EVERY_N_STAGES === 0;
}
...
function generateStage(stageNum) {
  ...
  const behavior = VARIANT_BEHAVIOR[variant] || VARIANT_BEHAVIOR.basic;
  const rage = isRageStage(stageNum);
  ...
  // Apply variant window multiplier
  attackWindow = Math.max(180, Math.floor(attackWindow * behavior.windowMult));
