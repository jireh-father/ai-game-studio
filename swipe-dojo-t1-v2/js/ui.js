...
class HUD {
  ...
  showVariantHint(variant) {
    const hints = {
      fast: 'FAST! Short window!',
      tank: 'TANK! Swipe x2!',
      tricky: 'TRICKY! Arrow changes!',
      boss: 'BOSS! 3-arrow combos!'
    };
    const hint = hints[variant];
    if (!hint) return;
    const txt = this.scene.add.text(GAME_WIDTH / 2, GAME_HEIGHT / 2 - 40, hint, {
      fontSize: '20px', fontFamily: 'Arial Black', color: '#FF6B00',
      stroke: '#000000', strokeThickness: 3
    }).setOrigin(0.5).setDepth(30);
    this.scene.tweens.add({
      targets: txt, alpha: 0, y: txt.y - 40, duration: 1200,
      delay: 600, onComplete: () => txt.destroy()
    });
  }
}
