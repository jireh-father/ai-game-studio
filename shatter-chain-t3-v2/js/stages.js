...
function generateWave(waveNumber, sessionSeed, dailyMod) {
  ...
  panels.forEach(p => {
    ...
    // Daily modifier: fragile glass
    if (dailyMod && dailyMod.allHp1) { p.hp = 1; p.type = 'normal'; p.w = 40; p.h = 30; }
    // Daily modifier: tiny panels
    if (dailyMod && dailyMod.panelScale) { p.w = Math.round((p.w || 40) * dailyMod.panelScale); p.h = Math.round((p.h || 30) * dailyMod.panelScale); }
  });
  ...
  return { panels, barriers, diff, dailyMod: dailyMod || null };
}
