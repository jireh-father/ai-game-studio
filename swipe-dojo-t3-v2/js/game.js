...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.lastStandActive = false; this.overkillDelay = false; this.lastBlockQuality = null;
    ...
    this.enemyNameText = this.add.text(GAME_WIDTH / 2, 110, '', { fontSize: '16px', fontFamily: 'Arial Black', color: PALETTE.uiText }).setOrigin(0.5).setDepth(11);
    ...
    if (!this.textures.exists('particle')) {
      const g = this.make.graphics({ add: false }); g.fillStyle(0xFFFFFF); g.fillRect(0, 0, 6, 6); g.generateTexture('particle', 6, 6); g.destroy();
    }
  ...
  _resolveBlock(quality, points) {
    if (quality !== 'late') { this.combo++; if (this.combo > this.maxCombo) this.maxCombo = this.combo; } else { this.combo = 0; }
    const lsMult = this.lastStandActive ? LAST_STAND.SCORE_MULT : 1;
    const gained = Math.floor(points * (quality !== 'late' ? Math.min(4, 1 + this.combo * 0.1) : 1) * lsMult);
    this.lastBlockQuality = quality;
    this.score += gained;
    this.hud.updateScore(this.score);
    this.hud.updateCombo(this.combo);
    audioSynth.playBlock(quality, this.combo);
    if (this.combo > 0 && this.combo % 5 === 0) { audioSynth.playComboMilestone(this.combo); this._comboMilestoneParticles(); this._showComboAnnouncer(this.combo); }
    this._floatingScore(gained, quality);
    this._blockJuice(quality);
    const wasAt1HP = this.currentEnemyHP === 1;
    this.currentEnemyHP--;
    this._updateHPBar();
    this.tweens.add({ targets: this.enemySprite, scaleX: 1.2, scaleY: 1.2, duration: 60, yoyo: true });
    this._blockParticles(quality);
    if (this.currentEnemyHP <= 0) { this._enemyDefeated(wasAt1HP, wasAt1HP && quality === 'perfect'); }
  ...
  _takeDamage() {
    ...
    if (this.lives <= 0) {
      this.lastStandActive = false; this._stopLastStandEffects();
      this._triggerDeath();
    } else {
      // v2: activate last stand at 1 HP
      if (this.lives === 1 && !this.lastStandActive) { this.lastStandActive = true; this._startLastStandEffects(); }
  ...
  _enemyDefeated(isFinishingBlow, isPerfectFinish) {
    const lsMult = this.lastStandActive ? LAST_STAND.SCORE_MULT : 1;
    const defeatBonus = Math.floor((SCORE.ENEMY_DEFEAT + this.combo * 50) * lsMult);
    this.score += defeatBonus;
    this._floatingScore(defeatBonus, 'perfect');
    audioSynth.playEnemyDeath();
    if (isFinishingBlow) {
      this._finishingBlowEffect(isPerfectFinish);
      if (isPerfectFinish) { this.score += FINISHING_BLOW.CRITICAL_BONUS; this._floatingScore(FINISHING_BLOW.CRITICAL_BONUS, 'perfect'); }
    }
    if (isPerfectFinish) { this.score += OVERKILL.BONUS_SCORE; this._showOverkillStamp(); this.overkillDelay = true; }
    if (this.lastStandActive) {
      this.lives += LAST_STAND.HEAL_AMOUNT; this.hud.updateLives(this.lives); this._showComebackText();
      if (this.lives > 1) { this.lastStandActive = false; this._stopLastStandEffects(); }
    }
  ...
  _stageClear() {
    ...
    this.score += Math.floor(clearBonus * lsMult);
    this.hud.updateScore(this.score);
    audioSynth.playStageClear();
    this._stageClearCelebration();
    audioSynth.setBPM(Math.min(150, 90 + Math.floor(this.stage / 5) * 2));
    this.stage++;
    const delay = this.overkillDelay ? 800 + OVERKILL.SPAWN_DELAY_MS : 800;
    this.overkillDelay = false;
    this.time.delayedCall(delay, () => this._startStage());
  }
