...
class EffectsManager {
  ...
  // Golden rush flash text
  goldenRushFlash() {
    const txt = this.scene.add.text(CFG.WIDTH / 2, CFG.ARENA_TOP + CFG.ARENA_HEIGHT / 2,
      '3X GOLD RUSH!', {
        fontSize: '56px', fontFamily: 'Arial', fontStyle: 'bold',
        color: CFG.GOLDEN_HEX, stroke: '#000', strokeThickness: 4,
      }).setOrigin(0.5).setDepth(120).setScale(0.3).setAlpha(1);
    this.scene.tweens.add({
      targets: txt, scaleX: 1.2, scaleY: 1.2, duration: 150,
      ease: 'Back.easeOut', yoyo: true, hold: 800,
      onComplete: () => {
        this.scene.tweens.add({ targets: txt, alpha: 0, duration: 300, onComplete: () => txt.destroy() });
      }
    });
    // Gold flash overlay
    const flash = this.scene.add.rectangle(CFG.WIDTH/2, CFG.HEIGHT/2, CFG.WIDTH, CFG.HEIGHT, CFG.GOLDEN_COLOR, 0.25).setDepth(115);
    this.scene.tweens.add({ targets: flash, alpha: 0, duration: 500, onComplete: () => flash.destroy() });
    this.sparks(CFG.WIDTH / 2, CFG.ARENA_TOP + CFG.ARENA_HEIGHT / 2, 0, 20);
  }

  // Bomb explosion effect
  bombExplosion(x, y) {
    // Expanding orange ring
    const ring = this.scene.add.circle(x, y, 10, CFG.BOMB_COLOR, 0.6).setDepth(95);
    ring.setStrokeStyle(3, 0xFFFF00);
    this.scene.tweens.add({
      targets: ring, scaleX: 8, scaleY: 8, alpha: 0, duration: 400,
      onComplete: () => ring.destroy()
    });
    // 20 orange/yellow sparks
    for (let i = 0; i < 20; i++) {
      const angle = Math.random() * Math.PI * 2;
      const speed = 100 + Math.random() * 250;
      const r = 2 + Math.random() * 4;
      const color = Math.random() > 0.5 ? CFG.BOMB_COLOR : 0xFFFF00;
      const spark = this.scene.add.circle(x, y, r, color, 1).setDepth(96);
      this.scene.tweens.add({
        targets: spark,
    ...
    const center = this.scene.add.circle(x, y, 40, CFG.COLOR.WHITE, 0.9).setDepth(97);
    this.scene.tweens.add({ targets: center, alpha: 0, scaleX: 2, scaleY: 2, duration: 150, onComplete: () => center.destroy() });
  }

  // Achievement toast at top
  achievementToast(label) {
    playAchievement();
    const txt = this.scene.add.text(CFG.WIDTH / 2, -30, `UNLOCKED: ${label}`, {
      fontSize: '16px', fontFamily: 'Arial', fontStyle: 'bold',
      color: CFG.GOLDEN_HEX, stroke: '#000', strokeThickness: 2,
      backgroundColor: 'rgba(0,0,0,0.7)', padding: { x: 12, y: 6 },
    }).setOrigin(0.5).setDepth(500);
    this.scene.tweens.add({
      targets: txt, y: 30, duration: 300, ease: 'Back.easeOut',
      hold: 1200,
      onComplete: () => {
        this.scene.tweens.add({ targets: txt, y: -30, alpha: 0, duration: 300, onComplete: () => txt.destroy() });
      }
    });
  }
}
