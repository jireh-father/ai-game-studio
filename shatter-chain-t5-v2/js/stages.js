...
function generateWave(waveNumber, sessionSeed) {
  ...
  // Assign glass types (including bomb and ice)
  const bombRatio = waveNumber >= CFG.BOMB_WAVE ? CFG.BOMB_RATIO : 0;
  const iceRatio = waveNumber >= CFG.ICE_WAVE ? CFG.ICE_RATIO : 0;

  panels.forEach(p => {
    const r = rng();
    if (r < bombRatio) {
      p.type = 'bomb'; p.hp = 1; p.w = CFG.BOMB_SIZE_W; p.h = CFG.BOMB_SIZE_H;
    } else if (r < bombRatio + iceRatio) {
      p.type = 'ice'; p.hp = 1; p.w = 40; p.h = 30;
    } else if (r < bombRatio + iceRatio + diff.armoredRatio) {
      p.type = 'armored'; p.hp = 3; p.w = 48; p.h = 38;
    } else if (r < bombRatio + iceRatio + diff.armoredRatio + diff.reinforcedRatio) {
      p.type = 'reinforced'; p.hp = 2; p.w = 44; p.h = 34;
    } else {
      p.type = 'normal'; p.hp = 1; p.w = 40; p.h = 30;
    }
  });
  ...
  // Select one golden panel (not bomb/ice)
  const normalPanels = panels.filter(p => p.type === 'normal' || p.type === 'reinforced');
  let goldenIndex = -1;
  if (normalPanels.length > 0) {
    const chosen = normalPanels[Math.floor(rng() * normalPanels.length)];
    goldenIndex = panels.indexOf(chosen);
    // Golden moves at wave 8+
    if (waveNumber >= CFG.GOLDEN_MOVE_WAVE) {
      chosen.goldenMoving = true;
    }
  }
