...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.glassBodies = [];
    this.glassSprites = [];
    wave.panels.forEach((p, i) => this.createGlassPanel(p, i === wave.goldenIndex));

    this.barriers = [];
    wave.barriers.forEach(b => this.createBarrier(b));

    this.goldenMultiplierActive = false; this.goldenResolved = false; this.goldenBody = null;
    this._frozenBall = null; this._frozenThisLaunch = false;
    this.shards = [];
  ...
  drawBackground() {
    const g = this.add.graphics().setDepth(0), size = 30;
    g.lineStyle(1, 0x1E1E35, 0.15);
    for (let r = 0; r < CFG.HEIGHT/(size*1.5)+1; r++) for (let c = 0; c < CFG.WIDTH/(size*1.73)+1; c++) g.strokeCircle(c*size*1.73+(r%2?size*0.866:0), r*size*1.5, size*0.5);
    g.lineStyle(2, CFG.COLOR.BORDER, 0.6); g.strokeRect(0, CFG.ARENA_TOP, CFG.WIDTH, CFG.ARENA_HEIGHT);
    g.lineStyle(1, 0x333355, 0.4); g.lineBetween(10, CFG.LAUNCH_ZONE_TOP, CFG.WIDTH-10, CFG.LAUNCH_ZONE_TOP);
  }

  createWalls() {
    const t = 20, o = { isStatic: true, label: 'wall', collisionFilter: { category: CFG.CAT_WALL, mask: CFG.CAT_BALL | CFG.CAT_SHARD } };
    this.matter.add.rectangle(CFG.WIDTH/2, -t/2, CFG.WIDTH+40, t, o); this.matter.add.rectangle(-t/2, CFG.HEIGHT/2, t, CFG.HEIGHT+40, o);
    this.matter.add.rectangle(CFG.WIDTH+t/2, CFG.HEIGHT/2, t, CFG.HEIGHT+40, o); this.matter.add.rectangle(CFG.WIDTH/2, CFG.HEIGHT+t/2+60, CFG.WIDTH+40, t, { ...o, label: 'floor' });
  }

  createGlassPanel(p, isGolden) {
    ...
    let color;
    if (isGolden) color = CFG.GOLDEN_COLOR;
    else if (p.type === 'bomb') color = CFG.BOMB_COLOR;
    else if (p.type === 'ice') color = CFG.ICE_COLOR;
    else if (p.type === 'armored') color = CFG.COLOR.ARMORED;
    else if (p.type === 'reinforced') color = CFG.COLOR.REINFORCED;
    else color = CFG.COLOR.GLASS;

    const sprite = this.add.rectangle(p.x, p.y, w, h, color, p.type === 'normal' || isGolden ? 0.85 : 0.9).setDepth(20);
    ...
    if (p.type === 'bomb') {
      const fuse = this.add.circle(p.x, p.y, 3, CFG.COLOR.WHITE).setDepth(22);
      this.tweens.add({ targets: fuse, alpha: 0.3, duration: 400, yoyo: true, repeat: -1 });
      sprite._fuse = fuse;
    } else if (p.type === 'ice') {
      const ice = this.add.graphics().setDepth(22);
      ice.lineStyle(0.5, CFG.COLOR.WHITE, 0.3);
      for (let i = -w/2+4; i < w/2; i += 6) { ice.lineBetween(p.x+i, p.y-h/2+2, p.x+i+h*0.5, p.y+h/2-2); ice.lineBetween(p.x+i, p.y+h/2-2, p.x+i+h*0.5, p.y-h/2+2); }
      sprite._icePattern = ice;
    }
    if (isGolden) {
      body._isGolden = true; this.goldenBody = body;
      const glow = this.add.circle(p.x, p.y, 24, CFG.GOLDEN_COLOR, 0.2).setDepth(19);
      this.tweens.add({ targets: glow, alpha: 0.05, scaleX: 1.3, scaleY: 1.3, duration: 800, yoyo: true, repeat: -1 });
      sprite._goldenGlow = glow;
      if (p.goldenMoving) { body.plugin.moving = true; body.plugin.moveSpeed = Math.PI*2/(CFG.GOLDEN_MOVE_PERIOD/1000); body.plugin.moveAmp = CFG.GOLDEN_MOVE_AMP; body.plugin.baseX = p.x; }
    }
  ...
  createHUD() {
    const gs = window.GameState, s = { fontSize: '14px', fontFamily: 'Arial', fontStyle: 'bold', color: CFG.COLOR.WHITE_HEX };
    ...
    this.ballIndicators = []; this.updateBallIndicators();
  }
  updateBallIndicators() {
    this.ballIndicators.forEach(b => b.destroy()); this.ballIndicators = [];
    const gs = window.GameState, total = this.waveDiff.balls, startX = CFG.WIDTH/2-(total-1)*18;
    for (let i = 0; i < total; i++) { const f = i < gs.ballsLeft; const c = this.add.circle(startX+i*36, CFG.HEIGHT-40, 10, f?CFG.COLOR.BALL:0x333355, f?1:0.4).setDepth(300); c.setStrokeStyle(f?1.5:1, f?CFG.COLOR.BALL_HI:0x555577); this.ballIndicators.push(c); }
  }
  showWaveStart() {
    const gs = window.GameState, label = this.waveDiff.isRest?'BONUS WAVE':this.waveDiff.isBoss?'BOSS WAVE':`WAVE ${gs.waveNumber}`;
    const color = this.waveDiff.isRest?CFG.COLOR.SCORE_HEX:this.waveDiff.isBoss?CFG.COLOR.DANGER_HEX:CFG.COLOR.WHITE_HEX;
    const txt = this.add.text(CFG.WIDTH/2, CFG.HEIGHT/2, label, { fontSize: '40px', fontFamily: 'Arial', fontStyle: 'bold', color, stroke: '#000', strokeThickness: 3 }).setOrigin(0.5).setDepth(400).setAlpha(0);
    this.tweens.add({ targets: txt, alpha: 1, duration: 200, hold: 400, yoyo: true, onComplete: () => txt.destroy() });
  }

  onPointerDown(pointer) {
    if (this.ballInFlight || !this.waveTimerActive || window.GameState.ballsLeft <= 0 || pointer.y < CFG.LAUNCH_ZONE_TOP) return;
    this.isAiming = true; this.aimStartX = pointer.x; this.aimStartY = pointer.y; this.lastInputTime = this.time.now;
  }
  onPointerMove(pointer) { if (!this.isAiming) return; this.drawTrajectory(pointer); this.lastInputTime = this.time.now; }
  onPointerUp(pointer) {
    if (!this.isAiming) return; this.isAiming = false; this.clearTrajectory();
    const dx = pointer.x-this.aimStartX, dy = pointer.y-this.aimStartY, dist = Math.sqrt(dx*dx+dy*dy);
    if (dist < CFG.BALL_DRAG_MIN) return; this.launchBall(dx, dy, dist); this.lastInputTime = this.time.now;
  }
  ...
  drawTrajectory(pointer) {
    ...
    const speed = Phaser.Math.Clamp(dist/30, CFG.BALL_SPEED_MIN, CFG.BALL_SPEED_MAX), angle = Math.atan2(dy, dx);
    const vx = Math.cos(angle)*speed, vy = Math.sin(angle)*speed, lx = CFG.WIDTH/2, ly = CFG.LAUNCH_ZONE_TOP-20;
    for (let i = 0; i < 8; i++) { const t = (i+1)*6; this.trajectoryDots.push(this.add.circle(lx+vx*t, ly+vy*t+0.2*t*t, 3, CFG.COLOR.WHITE, Math.max(0.1, 0.6-i*0.065)).setDepth(50)); }
  }
  ...
  update(time, delta) {
    ...
    for (const body of this.glassBodies) {
      ...
      if (body._sprite) {
        body._sprite.setPosition(bx, by);
        if (body._sprite._fuse) body._sprite._fuse.setPosition(bx, by);
        if (body._sprite._icePattern) body._sprite._icePattern.setPosition(bx - (body._sprite._icePattern._origX || 0), by - (body._sprite._icePattern._origY || 0));
        if (body._sprite._goldenGlow) body._sprite._goldenGlow.setPosition(bx, by);
      }
    }
